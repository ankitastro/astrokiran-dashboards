...
DB_CONFIG = {
    'host': os.getenv('DB_HOST', 'prod-example-postgres.postgres.database.azure.com'),
    'port': int(os.getenv('DB_PORT', 5432)),
    'database': os.getenv('DB_NAME', 'astrokiran'),
...
def execute_insert_returning(query: str, params: tuple = None):
    """Execute an INSERT with RETURNING clause and commit."""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(query, params) if params else cursor.execute(query)
    result = cursor.fetchall()
