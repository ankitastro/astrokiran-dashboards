...
from queries import PAYMENT_SUMMARY_QUERY, PAYMENT_BY_METHOD_QUERY, FAILED_PAYMENTS_QUERY, PENDING_PAYMENTS_QUERY
from fmt import colorize, pick_color, fmt_currency, fmt_percent, fmt_number, fmt_datetime, pad, GREEN, RED, YELLOW
...
def fetch_failed_payments(start_date=None, end_date=None) -> list:
    """Fetch failed payment details."""
    if start_date is None:
        start_date = date.today()
    if end_date is None:
        end_date = date.today()
    return execute_query(FAILED_PAYMENTS_QUERY, (start_date, end_date))


def fetch_pending_payments(start_date=None, end_date=None) -> list:
    """Fetch pending payment details."""
    if start_date is None:
        start_date = date.today()
    if end_date is None:
        end_date = date.today()
    return execute_query(PENDING_PAYMENTS_QUERY, (start_date, end_date))
...
def format_summary(data: tuple) -> tuple:
    ...
    success_rate = (float(successful) / int(total) * 100) if int(total) > 0 else 0
...
def format_failed_payment_row(row: tuple) -> tuple:
    """Format failed payment row."""
    order_id, user_id, name, phone, amount, method, created_at = row
    return (
        str(order_id),
        str(user_id),
        name[:20] if name else "-",
        phone or "-",
        pad(colorize(fmt_currency(float(amount)), RED)),
        method or "-",
        fmt_datetime(created_at) if created_at else "-"
    )


def format_pending_payment_row(row: tuple) -> tuple:
    """Format pending payment row."""
    order_id, user_id, name, phone, amount, method, created_at = row
    return (
        str(order_id),
        str(user_id),
        name[:20] if name else "-",
        phone or "-",
        pad(colorize(fmt_currency(float(amount)), YELLOW)),
        method or "-",
        fmt_datetime(created_at) if created_at else "-"
    )
...
class PaymentsView(BaseView):
    ...
    def get_containers(self) -> List[ContainerConfig]:
        return [
            ...
            ContainerConfig("failed-payments-container", "FAILED PAYMENTS", [
                TableConfig("failed-payments-table", [
                    "OrderID",
                    "UserID",
                    "Name",
                    "Phone",
            ...
            ContainerConfig("pending-payments-container", "PENDING PAYMENTS", [
                TableConfig("pending-payments-table", [
                    "OrderID",
                    "UserID",
                    "Name",
                    "Phone",
    ...
    def fetch_data(self, **kwargs) -> dict:
        ...
        return {
            ...
            'failed': fetch_failed_payments(start, end),
            'pending': fetch_pending_payments(start, end)
        }
    ...
    def format_rows(self, data: dict) -> dict:
        return {
            ...
            'failed-payments-table': [format_failed_payment_row(r) for r in data['failed']],
            'pending-payments-table': [format_pending_payment_row(r) for r in data['pending']]
        }
